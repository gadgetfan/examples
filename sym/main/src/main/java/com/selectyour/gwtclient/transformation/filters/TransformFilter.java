package com.selectyour.gwtclient.transformation.filters;

import com.selectyour.gwtclient.transformation.IBufferedImage;
import com.selectyour.gwtclient.transformation.IPixels;
...
public abstract class TransformFilter extends AbstractBufferedImageOp {
     ...
     */
    protected int edgeAction = ZERO;//TODO: make RGB_CLAMP, if working with photo ?;
    /**
    ...
    public IBufferedImage filter(IBufferedImage src, IBufferedImage dst) {
...
//		WritableRaster srcRaster = src.getRaster();
        ...
        if (dst == null) {
            dst = src.constructImage(transformedSpace.width, transformedSpace.height);
...
//		int[] inPixels = getRGB( src, 0, 0, width, height, null );
        IPixels inPixels = src.getPixels();
...
//		int[] outPixels = new int[outWidth];
        IPixels outPixels = dst.getPixels();
...
//					se = inPixels[i+srcWidth+1];
                    nw = inPixels.get(i);
                    ne = inPixels.get(i + 1);
                    sw = inPixels.get(i + srcWidth);
                    se = inPixels.get(i + srcWidth + 1);

                    outPixels.set(offset + x,
                            ImageMath.bilinearInterpolate(xWeight, yWeight, nw, ne, sw, se));//my code
                } else {
...
//					se = getPixel( inPixels, srcX+1, srcY+1, srcWidth, srcHeight );
                    outPixels.setTransparent(offset + x);//my code
                }
    ...
    final private int getPixel(int[] pixels, int x, int y, int width, int height) {
        if (x < 0 || x >= width || y < 0 || y >= height) {
            switch (edgeAction) {
                case ZERO:
                default:
                    return 0; //TODO: make alpha 0 (transparent) here. Add new const TRANSPARENT
//			case WRAP:
